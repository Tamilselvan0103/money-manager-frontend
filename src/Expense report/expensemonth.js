...
const ExpenseReportMonthly = ({ expense }) => {
  const [monthexpense, setmonthexpense] = useState([]);
  const [expensevalue, setExpensevalue] = useState();

  const getmonthlyexpensedata = async () => {
    try {
      const response = await fetch(`${expense}?month=${expensevalue}`, {
        method: "GET",
      });
  ...
  return (
    <div className="month">
      <NavBarFunction>
        ...
        />
        <button onClick={getmonthlyexpensedata} className="btn1">
          click
        </button>
...
export default ExpenseReportMonthly;
