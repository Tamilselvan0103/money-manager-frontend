...
const ExpenseReportweekly = ({ expense }) => {
  const [monthexpense, setmonthexpense] = useState([]);
  const [expensevalue, setExpensevalue] = useState();

  const getweeklyexpensedata = async () => {
    try {
      const response = await fetch(`${expense}?week=${expensevalue}`, {
        method: "GET",
      });
  ...
  return (
    <div className="month">
      <NavBarFunction>
        ...
        />
        <button onClick={getweeklyexpensedata} className="btn1">
          click
        </button>
...
export default ExpenseReportweekly;
