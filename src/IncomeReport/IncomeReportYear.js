...
const IncomeReportYearly = ({ income }) => {
  ...
  const getYearincomedata = async () => {
    try {
      const response = await fetch(`${income}?year=${incomevalue}`, {
        method: "GET",
      });
  ...
  return (
    <div className="month">
      <NavBarFunction>
        ...
        />
        <button onClick={getYearincomedata} className="btn1">
          click
        </button>
...
export default IncomeReportYearly;
